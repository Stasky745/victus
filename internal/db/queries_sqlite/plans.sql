...
VALUES (
    sqlc.arg(id), sqlc.arg(day_plan_id), sqlc.arg(category_id), sqlc.arg(meal_id), sqlc.arg(quantity),
    COALESCE(
        (SELECT MAX(sort_order) + 1 FROM day_plan_items WHERE day_plan_id = sqlc.arg(day_plan_id) AND category_id = sqlc.arg(category_id)),
        0
    )
)
RETURNING *;
...
UPDATE day_plan_items SET quantity = ? WHERE id = ?;

-- name: RemoveDayPlanItem :exec
DELETE FROM day_plan_items WHERE id = ?;
...
WHERE day_plan_items.day_plan_id = ?
...
VALUES (
    sqlc.arg(id), sqlc.arg(user_id), sqlc.arg(category_id), sqlc.arg(meal_id), sqlc.arg(quantity),
    COALESCE(
        (SELECT MAX(sort_order) + 1 FROM default_day_items WHERE user_id = sqlc.arg(user_id) AND category_id = sqlc.arg(category_id)),
        0
    )
)
RETURNING *;
...
DELETE FROM default_day_items WHERE id = ? AND user_id = ?;
