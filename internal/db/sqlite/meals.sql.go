...
WHERE meal_favorite_categories.meal_id IN (/*SLICE:meal_ids*/?)
...
func (q *Queries) ListFavoriteCategoriesForMeals(ctx context.Context, mealIds []uuid.UUID) ([]ListFavoriteCategoriesForMealsRow, error) {
	query := listFavoriteCategoriesForMeals
...
WHERE meal_label_assignments.meal_id IN (/*SLICE:meal_ids*/?)
...
UPDATE meals
SET name = ?1, recipe_url = ?2, serving_label = ?3,
    serving_amount = ?4, updated_at = CURRENT_TIMESTAMP
WHERE id = ?5
RETURNING id, name, source, source_ref, recipe_url, serving_label, serving_amount, created_by, created_at, updated_at
`
